...
const fetchPostDetail = async () => {
  axiosRequest
    .get('/post/' + routeInfo.params.postId, {})
    .then((response) => {
...
const handleLike = async () => {
  ...
  try {
    const response = await apiClient.post(
      `/post/${postDetail.value.postId}/like`,
      {},
      {
        headers: {},
      },
...
<template>
  <div class="container">
    ...
    <section class="comments-section" id="commentsSection">
      ...
      <!-- 댓글 작성 폼 -->
      <div class="comment-form" id="commentForm" v-show="isLoggedIn()">
        <form id="addCommentForm">
          <div class="form-group">
            <textarea id="commentContent" placeholder="댓글을 입력하세요..." required></textarea>
          </div>
          <div class="form-actions">
            <button type="button" class="btn btn-primary" @click="commentCreate">
      ...
      <div class="login-required" id="loginRequired" v-show="!isLoggedIn()">
        <p>댓글을 작성하려면 로그인이 필요합니다.|{{ isLoggedIn() }}|</p>
      </div>

      <!-- 댓글 목록 -->
      <div class="comments-list" id="commentsList" style="">
